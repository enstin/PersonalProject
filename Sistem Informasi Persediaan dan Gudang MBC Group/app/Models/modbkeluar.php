...
class modbkeluar extends Model
{
   ...
   {
      return $this->db->query("delete from dump_detbkeluar join bkeluar where id_gudang=g1");
   }
   ...
   public function tampilbkeluarwhere($id)
   {
      return $this->db->table('dump_detbkeluar')
         ->select('*')
         ->join('detbarang', 'dump_detbkeluar.id_detbarang=detbarang.id_detbarang')
         ->join('cr', 'cr.id_cr=detbarang.id_cr')
         ->where('id_detbkeluar', $id)
         ->get()->getResultArray();
   }
   public function cekdumptambah()
   {
      return $this->db->table('dump_detbkeluar')
         ->select('*')
         ->join('bkeluar', 'bkeluar.id_bkeluar=dump_detbkeluar.id_bkeluar')
         ->where("dump_detbkeluar.id_bkeluar = (SELECT bkeluar.id_bkeluar FROM bkeluar ORDER BY id_bkeluar DESC LIMIT 1)")
         ->where('bkeluar.id_gudang', 'g1')
         ->countAllResults();
   }
   public function cekdettambah()
   {
      return $this->db->table('detbkeluar')
         ...
         ->where("detbkeluar.id_bkeluar = (SELECT bkeluar.id_bkeluar FROM bkeluar ORDER BY id_bkeluar DESC LIMIT 1)")
         ->where('bkeluar.id_gudang', 'g1')
         ->countAllResults();
   }
