...
use App\Models\modpenyesuaianso_g2;
use CodeIgniter\HTTP\Request;
use mysqli;
use PDO;

class conpenyesuaian_g2 extends BaseController
{
	...
	{
		$this->model = new modpenyesuaianso_g2();
		$this->base_link = '/jurnal_g2';
	...
	public function trans($id)
	{
		...
		return redirect()->to('/jurnal/transaksi-penyesuaian/' . $id);
	}
	...
	public function tambah_item()
	{
		...
		$path = "/jurnal_g2/transaksi-detpenyesuaian/" . $id_det;
		// dd($data);
		try {
	...
	public function hapus_draft($id)
	{
		...
		$path = '/jurnal_g2/transaksi-detpenyesuaian/' . $id_detso;
		return redirect()->to($path);
	}
	public function simpan_draft()
	{
		...
		$d = session()->get('id_so');
		$pathh = "/jurnal_g2/transaksi-penyesuaian/" . $d;
		// dd($pathh);
	...
	public function selesai_draft()
	{
		...
		$pathh = "/jurnal_g2/transaksi-persetujuan/" . $d;
		// dd($pathh);
		return redirect()->to($pathh);
	}
	public function selesai_trans()
	{
		return redirect()->to('/jurnal_g2');
	}
	public function updateso()
	{
		...
		if ($cek > 0) {
			...
			return redirect()->to('/jurnal_g2/transaksi-penyesuaian/' . $d);
		}
	}
	//persetujuan
	...
	public function setujuitambah($id)
	{
		...
		foreach ($stok as $jumlah_stok) {
			...
			if ($jumlah_stok['convert'] == 'con1') {
				...
				return redirect()->to('/jurnal_g2/transaksi-detpersetujuan/' . $id_rdr);
