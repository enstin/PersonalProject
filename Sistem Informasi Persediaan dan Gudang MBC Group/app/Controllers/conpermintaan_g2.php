...
use App\Models\modpermintaan_g2;
use mysqli;
use PDO;

class conpermintaan_g2 extends BaseController
{
	...
	{
		$this->model = new modpermintaan_g2();
		$this->base_link = '/permintaan_g2';
	...
	public function viewpermintaan()
	{
		...
		//end generate
		session()->set([
			'id_permintaan_g2' => $set_id,
		]);
		session()->set([
			'tujuan' => 'g1',
		]);
		$data_insert = [
			'id_permintaan' => session()->get('id_permintaan_g2'),
		...
		// dd($data);
		$id_segment = session()->get('id_permintaan_g2');
	...
	public function simpendraft()
	{
		...
		$iddd 			= session()->get('id_permintaan_g2');
...
// 			die;
			...
			if ($sah) {
				...
				return redirect()->to('/permintaan_g2');
			}
		} else {
	...
	public function dikirim($id)
	{
		...
		// dd($insertdata);
		return redirect()->to('/permintaan_g2/kirim_2/' . $id);
	}
	public function dikirim_2($id)
	{
		...
		// dd($insertdata);
		return redirect()->to('/permintaan_g2/kirim_3/' . $id);
	}
	...
	public function viewdetail_diajukan_1($id)
	{
		...
		return redirect()->to('/permintaan_g2/detail_non/' . $id);
	}
	...
	public function tambah_dump_item_non()
	{
		...
		$jumlah = $this->request->getVar('jumlah');
		$cek_dump = $this->model->cek_dump($id_detbarang, $convert);
		$permintaan = session()->get('id_permintaan_g2');
		if ($cek_dump > 0) {
			echo session()->setFlashdata('pesan', 'Barang sudah pernah diinputkan dengan satuan tersebut');
			return redirect()->to('/permintaan_g2/detail_non/' . $permintaan);
		} else {
			if ($convert == 'con1') {
				if ($stok_base < $jumlah) {
					echo session()->setFlashdata('pesan', 'Stok barang tidak mencukupi');
