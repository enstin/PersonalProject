...
class conpenyesuaian extends BaseController
{
	...
	{
		$status_jabatan = session()->get('jabatan');
		if ($status_jabatan == 'Owner') {
			$tabel_display = $this->model->tampil_penyesuaian_owner();
		} else {
			$tabel_display = $this->model->tampil_penyesuaian_kepgudang();
		}
		$data = [
			'title' => $this->judul,
			'tb_sesuai' => $tabel_display,
	...
	public function tambah_item()
	{
		...
		$path = "/jurnal/transaksi-detpenyesuaian/" . $id_det;
		// dd($data);
		try {
	...
	public function hapus_draft($id)
	{
		...
		$path = '/jurnal/transaksi-detpenyesuaian/' . $id_detso;
		return redirect()->to($path);
	}
	...
	public function updateso()
	{
		...
		if ($cek > 0) {
			...
			return redirect()->to('/jurnal/transaksi-penyesuaian/' . $d);
		}
	}
	//persetujuan
	...
	public function setujuitambah($id)
	{
		...
		foreach ($stok as $jumlah_stok) {
			...
			if ($jumlah_stok['convert'] == 'con1') {
				...
				return redirect()->to('/jurnal/transaksi-detpersetujuan/' . $id_rdr);
