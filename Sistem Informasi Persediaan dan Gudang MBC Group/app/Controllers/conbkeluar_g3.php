...
use App\Models\modbkeluar_g3;
use CodeIgniter\HTTP\Request;
use Config\Database;
use mysqli;
use PDO;

class conbkeluar_g3 extends BaseController
{
	...
	{
		$this->model = new modbkeluar_g3();
		$this->base_link = '/bkeluar_g3';
	...
	public function proseskeluar()
	{
		...
		$id_bkeluar = session()->get('id_bkeluar_g3');
	...
	public function tambah_dump_item()
	{
		...
		if ($cek_dump > 0) {
			...
			return redirect()->to('/bkeluar_g3/draftbkeluar');
		} else {
			if ($convert == 'con1') {
				if ($stok_base < $jumlah) {
	...
	public function update_item($id)
	{
		...
		foreach ($tabel as $detbkeluar) {
			if ($detbkeluar['convert'] == 'con1') {
				...
				if ($jumlah > $detbkeluar['stok_con1'] && $detbkeluar['stok_base'] < 1) {
					...
					$this->model->dump_update_item($data, $id_mas);
					return redirect()->to('/bkeluar_g3/draftbkeluar');
				} elseif ($jumlah <= $detbkeluar['stok_con1']) {
					$id_mas = [
						'id_detbkeluar' => $id
					];
					$data = [
				...
				if ($jumlah > $detbkeluar['stok_con2'] && $detbkeluar['stok_base'] < 1 && $detbkeluar['stok_con1'] < 1) {
					...
					$this->model->dump_update_item($data, $id_mas);
					return redirect()->to('/bkeluar_g3/draftbkeluar');
				} elseif ($jumlah <= $detbkeluar['stok_con2']) {
