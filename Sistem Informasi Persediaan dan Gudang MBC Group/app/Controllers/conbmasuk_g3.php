...
use App\Models\modbmasuk_g3;
use mysqli;
use PDO;

class conbmasuk_g3 extends BaseController
{
	...
	{
		$this->model = new modbmasuk_g3();
		$this->base_link = '/bmasuk_g3';
	}
	...
	public function prosesmasuk()
	{
		...
		$id = session()->get('id_permintaan');
		$insertdata = $this->model->transaksimasuk($id);
		foreach ($insertdata as $masuk) {
			$id_explode = explode('-', $masuk['id_detbarang']);
			$id_detbarang = $id_explode[0] . '-' . $id_explode[1] . '-' . $id_explode[2] . '-' . $id_explode[3] . '-' . $id_explode[4] . '-' . $id_explode[5] . '-g3';
	...
	public function prosesmasuk_non()
	{
		...
		$idd = [
			'id_bmasuk' => session()->get('id_masuk_g3'),
		];
		// dd($stok_con1_masuk);
	...
	public function tambah_dump_item()
	{
		...
		if ($cek_dump > 0) {
			...
			return redirect()->to('/bmasuk_g3/tambah_draftbmasuk');
		} else {
			if ($convert == 'con1') {
				date_default_timezone_set('Asia/Jakarta');
				$id_det = date('ymdHis'); //201120
				...
				$data = [
					...
					'id_masuk' => session()->get('id_masuk_g3'),
					'jumlah' => $jumlah,
					'convert' => $convert
				];
	...
	public function ubah_draft($id)
	{
		...
		return redirect()->to('/bmasuk_g3/draft');
	}
